...
![LinkVision Dashboard](screenshots/screen_1.png)
...
### 7. Первый вход и управление паролем

После запуска перейдите по адресу `http://127.0.0.1:5000`. В системе автоматически создаётся администратор:

- Логин: `admin`
- Пароль: **генерируется случайный** при первом запуске

#### Как получить временный пароль

1. Запустите приложение: `python app.py`
2. В консоли появится строка:
   ```
   ============================================================
     Temporary admin password: a1b2c3d4e5f6g7h8
   ============================================================
   ```
3. Скопируйте пароль и войдите в систему
4. Система **обязательно** перенаправит вас на страницу смены пароля (`/auth/change-password`)
5. Введите временный пароль, придумайте новый и подтвердите

⚠️ **Важно:** Пароль показывается **только один раз** в консоли при первом запуске. Он **не** сохраняется в лог-файлах. Если вы закрыли консоль и не сохранили пароль — см. ниже.

#### Как сбросить пароль администратора

Если вы забыли временный пароль или не сохранили его:

**Вариант 1: Через БД (самый быстрый)**
```bash
python3 -c "
...
with app.app_context():
    ...
    admin.must_change_password = False  # опционально: отключить требование смены
...
**Вариант 2: Через fix_db.py (если есть доступ к скрипту)**
Добавьте в `fix_db.py` после создания бэкапа:
```python
cursor.execute("UPDATE user SET must_change_password = 0")
conn.commit()
```
И запустите: `./fix_db.py`

**Вариант 3: Удалить админа и создать нового**
```bash
sqlite3 webnetmap.db "DELETE FROM user WHERE username = 'admin';"
python app.py  # при первом запуске создастся новый админ с новым паролем в консоли
```

После сброса войдите в систему — система **не** потребует смену пароля, если вы установили `must_change_password = False`.
...
# - Добавление колонки must_change_password к user
