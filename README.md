...
2. Найдите в консоли строку:
   ```
   ⚠️ Создан администратор admin. Временный пароль: <сгенерированный_пароль>
   ```
...
⚠️ **Важно:** Пароль показывается только один раз в логах. Если вы его потеряли — см. ниже.
...
python app.py  # при первом запуске создастся новый админ с новым паролем в логах
```
