...
- [ ] 1.1 `monitor.py`: joinedload(ips) + один контекст + словарь + `device_status_batch`
- [ ] 1.2 `undoRedo.js`: bulk `/api/devices/positions`
- [ ] 2.1 убрать дубль `cy.on('pan zoom')` в `core.js`
- [ ] 2.2 `applyGrayStyle` — один `.style({...})`
- [ ] 2.3 применение стилей при загрузке в `cy.batch()`
- [ ] 2.4 `pulse.js` — цикл в `cy.batch()`
- [ ] 2.5 вернуть подавление hover-подсветки во время рамки
- [ ] 2.6 `export_map_data` — joinedload
- [ ] 2.7 `get_map_groups` — `func.count`
- [ ] 2.8 `import_map` — кэш типов
- [ ] 3.1 удалить мёртвый `frag`
- [ ] 3.2 статические импорты вместо `import()`
- [ ] 3.3 cleanup слушателей/таймеров при reload
- [ ] 3.4 индексы `type_id`, `Group.map_id` (+ миграция)
- [ ] A. точечные сокет-события (отдельная задача)
