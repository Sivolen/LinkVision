...
def test_map_lock():
    ...
    maps = response.json()
    if not maps:
        print("⚠️  Нет карт для тестирования")
        return
...
def test_map_permissions():
    ...
    print("ℹ️  Для добавления разрешения используйте:")
...
def main():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("\n⚠️  API недоступно. Запустите сервер для полного тестирования.")
    except requests.exceptions.ConnectionError:
        print("\n⚠️  Сервер не запущен. Запустите LinkVision для тестирования API.")
